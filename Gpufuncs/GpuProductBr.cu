...
// produce the sum or mean of each array
__global__ void breduce_float(const float * __restrict__ idata1, const float * __restrict__ idata2, float * __restrict__ odata, const int numst, const int pbasis){
...
__global__ void breduce_float(const cuFloatComplex * __restrict__ idata1, const cuFloatComplex * __restrict__ idata2, float * __restrict__ odata, const int numst, const int pbasis){
  ...
  size_t pidx = blockIdx.x*nTPB;
      ...
      for(size_t ns = 0;ns < numst;ns++)
      {
          ...
          if(poffset < pbasis)
          {
              sum += cuCrealf(cuCmulf(idata1[offset], cuConjf(idata2[offset])));
          }
      }
      if(poffset < pbasis)
          odata[poffset] = sum;
...
{
  int nblocks = pbasis / nTPB + 1;
  breduce_float<<<nblocks, nTPB>>>((cuComplex*)in1, (cuComplex*)in2, out, numst, pbasis);
}
