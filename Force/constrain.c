		...
		case 5:   /* NEB tangent to higher energy adjacent image with climbing/descending extrema */
			{
                ...
                if ( Mag_T == 0 ) {
                    error_handler("Image collision this(%s) image.", pct.thisimg);
                } else {
                    Mag_T = sqrt(Mag_T);
                }
		...
		case 4:                    /* NEB tangent to higher energy adjacent image */
			{
				...
				Mag_L = Mag_L > 0.0 ? sqrt(Mag_L) : error_handler("Left image(%s) collision in NEB", pct.thisimg);
				Mag_R = Mag_R > 0.0 ? sqrt(Mag_R) : error_handler("Right image(%s) collision in NEB", pct.thisimg);
