        ...
        double * vxc, double *vxc_in, double * vnuc, Kpoint<OrbitalType> **Kptr)
{
    ...
    rmg::reduce (force_tmp, size1, pct.spin_comm);
    ...
    if(ct.ldaU_mode != LDA_PLUS_U_NONE)
    {
        ...
        rmg::reduce (force_tmp, size1, pct.kpsub_comm);
        rmg::reduce (force_tmp, size1, pct.spin_comm);
        for(int i = 0; i < num_ions * 3; i++) force_sum[i] += force_tmp[i];
    ...
    rmg::reduce (force_sum, size1, pct.grid_comm);
...
void output_force(double *force_tmp, char *desc)    
{
    ...
    rmg::reduce (force_tmp, size1, pct.grid_comm);
    if (pct.imgpe == 0)
    {
        rmg::printlog ("\n\n %s", desc);
