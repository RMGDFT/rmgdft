                                            ...
                                            rho(rho_in), rhocore(rhocore_in), rhoc(rhoc_in)
{
    ...
    rmg::printlog("\n Total number of states: %d", ct.num_states);
    rmg::printlog("\n Number of states frozen in TDDFT:  %d", ct.tddft_start_state);
    rmg::printlog("\n Number of states in TDDFT propagation:  %d", numst);
    rmg::printlog("\n Number of states discarded for TDDFT:  %d", ct.num_states - ct.tddft_start_state -numst);
    if(pct.gridpe == 0) {
    ...
    occ_fi = (FILE **)malloc(ct.num_kpts_pe * sizeof(FILE*));

    std::filesystem::create_directory("TDDFT_OCC");
 
    for(int kpt = 0; kpt < ct.num_kpts_pe; kpt++)
    {
        occ_fi[kpt] = NULL;
        int kpt_glob = kpt + pct.kstart;
        filename = "TDDFT_OCC/"+std::string(ct.basename) +"_spin" +std::to_string(pct.spinpe)+ "_kpt" + std::to_string(kpt_glob) + "_occ.dat";
        if(pct.gridpe == 0)
        {
            occ_fi[kpt] = fopen(filename.c_str(), "w");
        }

        MatDiagGet((MatrixType *)Kptr[kpt]->Pn0_cpu, diag_elem, numst, *Sp);
        ...
        if(pct.gridpe == 0)
        {
            ...
            for(int i = vbm-ct.tddft_start_state -5; i < vbm-ct.tddft_start_state +8; i++) 
                if(i >= 0 && i <numst) fprintf(occ_fi[kpt], " %8.4f ",diag_elem[i]);
...
void rmg::tddft<OrbitalType, MatrixType>::tddft_md(void)
{
    ...
    if(ct.tddft_energy)
    {
        RT1 = new RmgTimer("2-TDDFT: energy init");
    ...
    for(int tddft_steps = 0; tddft_steps < ct.tddft_steps; tddft_steps++)
    {
        ...
        while (err > thrs_dHmat &&  iter_scf <  Max_iter_scf)  {
            ...
            RT1 = new RmgTimer("2-TDDFT: dipole");
            get_dipole(rho.data(), rhoc.data(), dipole_tot);
            if(ct.dipole_corr[0]+ct.dipole_corr[1]+ct.dipole_corr[2] >0)
            {
                DipoleCorrection(dipole_tot,  vh_dipole.data());
        ...
        {   
            ...
            if(pct.gridpe == 0)
            {
                for(int kpt = 0; kpt < ct.num_kpts_pe; kpt++) fflush(occ_fi[kpt]);
            }
            delete RT2a;
        }
    ...
    if(ct.forceflag == TDDFT_CVE)
    {
        RT1 = new RmgTimer("2-TDDFT: Force");
        ct.fpt[0] = 0;
        ct.fpt[1] = 1;
        ct.fpt[2] = 2;
        ct.fpt[3] = 3;
...
rmg::tddft<OrbitalType, MatrixType>::~tddft(void)
{
    ...
    for(int kpt = 0; kpt < ct.num_kpts_pe; kpt++)
    {
        if(occ_fi[kpt] != NULL) fclose(occ_fi[kpt]);
    }
