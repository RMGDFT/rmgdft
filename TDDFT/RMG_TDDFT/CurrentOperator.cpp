            ...
            for(int i = 0; i < px0_grid; i++)
            {
                for(int j = 0; j < py0_grid; j++)
                {
                    for(int k = 0; k < pz0_grid; k++)
                    {
                        ...
                        psi_x[idx] = psi[st_start * pbasis_noncol + idx] * xcrt[0] * kptr->Kstates[st_start + st1].eig[0];
                        psi_y[idx] = psi[st_start * pbasis_noncol + idx] * xcrt[1] * kptr->Kstates[st_start + st1].eig[0];
                        psi_z[idx] = psi[st_start * pbasis_noncol + idx] * xcrt[2] * kptr->Kstates[st_start + st1].eig[0];
