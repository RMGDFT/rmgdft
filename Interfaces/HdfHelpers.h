...
namespace hdfHelper {
  ...
  template<>
    hid_t inline getHdfDataType<int>(const int& val) {
    return H5T_NATIVE_INT;
  }
  template<>
    hid_t inline getHdfDataType<float>(const float& val) {
    return H5T_NATIVE_FLOAT;
  }
  template<>
    hid_t inline getHdfDataType<double>(const double& val) {
    return H5T_NATIVE_DOUBLE;
  }
    
  // note currently only support int,float and double
  template<typename T>
  herr_t inline writeNumsToHDF(const std::string& fieldName, const std::vector<T>& data, hid_t loc,
  ...
  template<typename T>
  herr_t inline writeNumsToHDF(const std::string& fieldName, T data, hid_t loc) {
    std::vector<T> tmp{data};
    return writeNumsToHDF(fieldName, tmp, loc);
  }

  herr_t inline writeStringToHDF(const std::string& fieldName, const std::string& str, hid_t loc) {
    hsize_t ns = 1;
  ...
  hid_t inline makeHDFGroup(const std::string& gName, hid_t location) {
