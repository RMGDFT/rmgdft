  ...
  for(int iy=0;iy<iyy;iy++){
    for(int iz=0;iz<izz;iz++){
        ...
        if(width == 4){
            if((ix==0)||(iy==0)||(iz==0))a[ix*iyy*izz+iy*izz+iz] = 0.0;
            if((ix==(ixx-1))||(iy==(iyy-1))||(iz==(izz-1)))a[ix*iyy*izz+iy*izz+iz] = 0.0;
            if((ix==1)||(iy==1)||(iz==1))a[ix*iyy*izz+iy*izz+iz] *= 0.25;
            if((ix==(ixx-2))||(iy==(iyy-2))||(iz==(izz-2)))a[ix*iyy*izz+iy*izz+iz] *= 0.25;
            if((ix==2)||(iy==2)||(iz==2))a[ix*iyy*izz+iy*izz+iz] *= 0.5;
            if((ix==(ixx-3))||(iy==(iyy-3))||(iz==(izz-3)))a[ix*iyy*izz+iy*izz+iz] *= 0.5;
            if((ix==3)||(iy==3)||(iz==3))a[ix*iyy*izz+iy*izz+iz] *= 0.75;
            if((ix==(ixx-4))||(iy==(iyy-4))||(iz==(izz-4)))a[ix*iyy*izz+iy*izz+iz] *= 0.75;
        }
