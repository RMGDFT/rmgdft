...
void get_dnmpsi(STATE *sp, double *kbpsi_one_state, double *kbpsi_res_one_state, double *work)
{
    ...
    double *prjptr;
    int ion2, nh;
    double *ddd, *kbpsi_one_ion, *qnm_weight;
    double *qqq, *kbpsi_res_one_ion;
    double *prj_sum;
    ...
    for (ion2 = 0; ion2 < pct.n_ion_center; ion2++)
    {
        ...
        kbpsi_res_one_ion = &kbpsi_res_one_state[ipindex];
        ...
        for (ip1 = 0; ip1 < nh; ip1++)
        {
            ...
            for (ip2 = 0; ip2 < nh; ip2++)
            {
                ...
                qsum -= ddd[ip1 * nh + ip2] * kbpsi_one_ion[ip2];
                qsum += 0.5*qqq[ip1 * nh + ip2] * kbpsi_res_one_ion[ip2];
            }
