...
void write_restart (char *name, double * vh, double *vxc, double *vh_old, double *vxc_old,  double * rho, double *rho_oppo, STATE *states)
{
    ...
    {
	if (pct.imgpe == 0)
	    rmg::printlog ("write_data: Output file given as /dev/null, no restart data written ...\n");
	return;
    }
	...
	rmg::printlog ("write_data: Restart file %s opened...\n", name);
	...
	for (ion = 0; ion < ct.num_ions; ion++)
	{
	    ...
	    fprintf(fhandle,"\n %s %#15.12g %#15.12g %#15.12g %d %d %d %f", sp->atomic_symbol, iptr->crds[0], iptr->crds[1], 
                iptr->crds[2], iptr->movable[0], iptr->movable[1], iptr->movable[2], iptr->init_spin_rho);
	}
