...
void local_current ()
{
    ...
    int iene, icond;
    std::complex<double> *H_tri,*S_tri, *g;
    ...
    std::complex<double> *H10, *S10, *H01, *H00, *S01, *S00;
    ...
    std::complex<double> *rho_mn, *green_C_non, *Gless, *Glesst;
...
/*======================== Reading Matrices ===============================*/
    ...
    my_malloc_init( S_tri, ntot, std::complex<double> );
    ...
    my_malloc_init( green_C_non, maxcol * totrow, std::complex<double> ); 
    my_malloc_init( rho_mn, ntot, std::complex<double> );

    i = pmo.mxllda_cond[1] * pmo.mxlocc_cond[1]; 
    my_malloc_init( Gless, i, std::complex<double> );
    my_malloc_init( Glesst, i, std::complex<double> );
    ...
    my_malloc_init( g,    idx, std::complex<double> );
    my_malloc_init( ch0,  idx, std::complex<double> );
    my_malloc_init( ch01,  idx, std::complex<double> );
    my_malloc_init( ch10,  idx, std::complex<double> );
    my_malloc_init( H00,  idx, std::complex<double> );
    my_malloc_init( H01,  idx, std::complex<double> );
    my_malloc_init( H10,  idx, std::complex<double> );
    my_malloc_init( S00,  idx, std::complex<double> );
    my_malloc_init( S01,  idx, std::complex<double> );
    my_malloc_init( S10,  idx, std::complex<double> );
    ...
    for (icond = 0; icond < ct.num_cond_curve; icond++)
    {
        ...
        my_malloc_init( Gamma1, nC_11, std::complex<double> ); 
        my_malloc_init( Gamma2, nC_22, std::complex<double> ); 

        nC_max = std::max(nC_11, nC_22);
        ...
        for (iene = pmo.myblacs; iene < E_POINTS; iene += pmo.npe_energy)
        {
            ...
            for(kp = 0; kp < nkp_tot; kp++)
            {
                ...
                for (i = 0; i < ntot; i++)
                {
                    H_tri[i] = std::real(ene) * S_tri[i] - H_tri[i] * Ha_eV;
                }
