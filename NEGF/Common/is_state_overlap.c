 ...
 **    $Id: is_state_overlap.c 1242 2011-02-02 18:55:23Z luw $    **
******************************************************************************/
...
void is_state_overlap (STATE * states)
{
    ...
    if (pct.thispe == 0)
    {
        printf ("\n state_overlap_or_not \n");
        for (state1 = 0; state1 < ct.num_states * ct.num_states; state1++)
        {
            if (state1 % 20 == 0)
                printf ("\n");
            printf (" %d ", state_overlap_or_not[state1]);
        }
    }
#endif
