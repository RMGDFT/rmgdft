...
void print_status (STATE * states, double * vh, double * vxc, double * vnuc, double * vh_old, char *msg)
{
    ...
    for (i = 0; i < get_P0_BASIS(); i++)
        total += vh[i];
    rmg::printlog ("PE: %d, vh total: %22.16f \n", pct.gridpe, total);

    total = 0;
    for (i = 0; i < get_P0_BASIS(); i++)
        total += vxc[i];
    rmg::printlog ("PE: %d, vxc total: %22.16f \n", pct.gridpe, total);

    total = 0;
    for (i = 0; i < get_P0_BASIS(); i++)
        total += vnuc[i];
    rmg::printlog ("PE: %d, vnuc total: %22.16f \n", pct.gridpe, total);

    total = 0;
    for (i = 0; i < get_P0_BASIS(); i++)
        total += vh_old[i];
    rmg::printlog ("PE: %d, vh_old total: %22.16f \n", pct.gridpe, total);
    ...
    for (i = ct.state_begin; i < ct.state_end; i++)
    {
        ...
        rmg::printlog ("PE: %d, state: %d, orbital total: %22.16f \n", pct.gridpe, i, total);
...
void print_data_to_file (int size, double *data, char *filename)
{
    ...
    if (file == NULL)
        rmg::error("Unable to open file to write ");
