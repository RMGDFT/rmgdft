...
void matrix_inverse_blocknm (std::complex<double> * H_tri_host, int N_blocks, int * ni,
...
/*  find the maximum dimension of the blocks  */
    ...
    nz = maxrow * maxcol;
    my_malloc_init( temp_host, nz * 5, std::complex<double> );
    my_malloc_init( Imatrix_host, nz, std::complex<double> );  
    ...
    RmgTimer *RT5 = new RmgTimer("data copy cpu-gpu");
    setvector_host_device (pmo.ntot, sizeof(std::complex<double>), H_tri_host, ione, ct.gpu_Htri, ione);
    delete RT5;
...
/*  iterate to get one more block  */
    ...
    RmgTimer *RT6 = new RmgTimer("data copy gpu-cpu");
    getvector_device_host (nz, sizeof(std::complex<double>), Green_C, ione, Green_C_host, ione);
    delete RT6;
    my_free(n_begin1);
    my_free( temp_host);
