 ...
 **    $Id: init_qfunct.c 1242 2011-02-02 18:55:23Z luw $    **
******************************************************************************/
...
/*		sp->qdim = 2 * RHO_NX * (it1 / 2) + 1;*/
        ...
        {
            printf ("qradius=%f  hmingrid=%e  rho_nx=%d  qdim=%d\n", sp->qradius, ct.hmingrid,
                    RHO_NX, sp->qdim);
        ...
        t1 = sp->qdim / RHO_NX + 1;
        sp->drqlig = sqrt (3.0) * (t1 + 1.0) * ct.hmaxgrid / 2.0;
        ...
        for (i = 0; i < sp->nbeta; i++)
        {
            ...
            for (j = i; j < sp->nbeta; j++)
            {
                ...
                for (ll = fabs (il - jl); ll <= il + jl; ll = ll + 2)
                {
                    ...
                    qnmlig_tpr[0] = 2.0 * qnmlig_tpr[1] - qnmlig_tpr[2];
                    if (ll)
                        qnmlig_tpr[0] = 0.0;
                    drqnmlig_tpr[1] = 2.0 * drqnmlig_tpr[2] - drqnmlig_tpr[3];
                    drqnmlig_tpr[0] = 2.0 * drqnmlig_tpr[1] - drqnmlig_tpr[2];
