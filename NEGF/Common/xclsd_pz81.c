 ...
 **    $Id: xclsd_pz81.c 1242 2011-02-02 18:55:23Z luw $    **
******************************************************************************/
...
REAL e_lsd_pz (REAL rho_up, REAL rho_dn)
{
    ...
    tmp = 9.0 * PI / 4.0;
    tmp = -(3.0 / (4.0 * PI)) * pow (tmp, 0.3333333333333333333333333e0);
    Ex_U = tmp / rs;
    ...
    if (rs >= 1.0)
    {
        ...
        Bpr_U = 1.0 + beta1_U * sqrt_rs + beta2_U * rs;
        Ec_U = gamma_U / Bpr_U;
    ...
    if (rs >= 1.0)
    {
        ...
        Bpr_P = 1.0 + beta1_P * sqrt_rs + beta2_P * rs;
        Ec_P = gamma_P / Bpr_P;
...
void mu_lsd_pz (REAL rho_up, REAL rho_dn, REAL * Vxc_up, REAL * Vxc_dn)
{
    ...
    if (rs >= 1.0)
    {
        ...
        Bpr_U = 1.0 + beta1_U * sqrt_rs + beta2_U * rs;
        Ec_U = gamma_U / Bpr_U;
        tmp = 1.0 + (7.0 / 6.0) * beta1_U * sqrt_rs + (4.0 / 3.0) * beta2_U * rs;
        Vc_U = Ec_U * tmp / Bpr_U;
    ...
    {                           /* if rs < 1 */
        ...
        log_rs = log (rs);
        Ec_U = A_U * log_rs + B_U + C_U * rs * log_rs + D_U * rs;
        Vc_U = Ec_U - (A_U + C_U * rs * (log_rs + 1.0) + D_U * rs) / 3.0;
    ...
    if (rs >= 1.0)
    {
        ...
        Bpr_P = 1.0 + beta1_P * sqrt_rs + beta2_P * rs;
        Ec_P = gamma_P / Bpr_P;
        tmp = 1.0 + (7.0 / 6.0) * beta1_P * sqrt_rs + (4.0 / 3.0) * beta2_P * rs;
        Vc_P = Ec_P * tmp / Bpr_P;
    ...
    {                           /* if rs < 1 */
        ...
        Ec_P = A_P * log_rs + B_P + C_P * rs * log_rs + D_P * rs;
        Vc_P = Ec_P - (A_P + C_P * rs * (log_rs + 1.0) + D_P * rs) / 3.0;
    ...
    E = (Exc_P - Exc_U) * df;
    *Vxc_up = VVxc + (1.0 - zeta) * E;
    *Vxc_dn = VVxc - (1.0 + zeta) * E;
