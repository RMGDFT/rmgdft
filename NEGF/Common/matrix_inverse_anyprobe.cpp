        ...
        std::complex<double> * G_row_host, std::complex<double> *G_col_host)
{
    ...
    my_malloc_init( Imatrix_host, maxrow * maxcol, std::complex<double> );
    my_malloc_init( Hii_host, maxrow * maxcol, std::complex<double> );
    my_malloc_init( Gii_host, maxrow * maxcol, std::complex<double> );
    my_malloc_init( temp_host, maxrow * maxcol, std::complex<double> );
    ...
    if(m < N - 1 )  
    {
        ...
        nz= ni[m] * ni[m];
        setvector_host_device (nz, sizeof(std::complex<double>), Imatrix_host, ione, ct.gpu_Imatrix, ione);
