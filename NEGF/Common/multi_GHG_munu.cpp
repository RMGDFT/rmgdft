...
void multi_GHG_munu (double *GHG_tri, double *GHG_en_tri)
{
    ...
    std::complex<double> *green_C, *complex_H, *temp_matrix_tri, *temp_matrix1;
    std::complex<double> *sigma_sum1, *sigma_sum2, *sigma_L;
    ...
    my_malloc_init( temp_matrix_tri, ntot, std::complex<double> );

    size = ct.num_states * ct.num_states;
    my_malloc_init( green_C, size, std::complex<double> );
    my_malloc_init( complex_H, size, std::complex<double> );
    my_malloc_init( temp_matrix1, size, std::complex<double> );
    ...
    for (iprobe = 1; iprobe <= cei.num_probe; iprobe++)
    {
        ...
        for (iene = pct.gridpe; iene < lcr[iprobe].nenergy; iene += pct.grid_npes)
        {
            ...
            for(idx1 = 0; idx1 < ntot; idx1++)
            {
                GHG_tri[idx1] +=  std::imag(weight * temp_matrix_tri[idx1]) ;
                GHG_en_tri[idx1] += std::imag ( weight * temp_matrix_tri[idx1] * ene);
            }
    ...
    if (ct.runflag == 113)
    {
        my_malloc_init( sigma_L, size, std::complex<double> );

        for (iprobe = 1; iprobe <= cei.num_probe; iprobe++)
        {

            /*   parallel for the processor on energy grid */
            for (iene = pct.gridpe; iene < lcr[iprobe].lcr_ne[0].nenergy_ne; iene += pct.grid_npes)
            {
                ...
                for(idx1 = 0; idx1 < ntot; idx1++)
                {
                    GHG_tri[idx1] -= 2.0 /PI * std::real(weight * temp_matrix_tri[idx1]);
                    GHG_en_tri[idx1] -= 2.0 /PI * std::real(weight * temp_matrix_tri[idx1] * ene); 
                }
