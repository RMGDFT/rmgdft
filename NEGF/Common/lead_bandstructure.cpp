...
//void PZHEGVX (int *, char *, char *, char *, int*, std::complex<double> *, int*, int*,
//        int *, std::complex<double> *, int*, int*, int*, double *, double*, int*, int*,
//        double *, int*, int*, double*,double*, std::complex<double> *, int*,int*, int*,
...
void lead_bandstructure ()
{
    ...
    std::complex<double> *H10, *S10, *H01, *H00, *S01, *S00, *HCL, *SCL;

    std::complex<double> ctem1, ctem2;
    ...
    my_malloc_init( matH, mxllda * mxlocc, std::complex<double> );
    ...
    my_malloc_init( SCL,  idx, std::complex<double> );
    my_malloc_init( HCL,  idx, std::complex<double> );
    ...
    my_malloc_init( WORK, LWORK, std::complex<double> );
   ...
   // kvecz = 0.0;
    ...
    for (ik = pmo.myblacs; ik < kpoints[0]; ik += pmo.npe_energy)
    {
        ...
        ctem1 = std::exp(I*kvec);
        ctem2 = std::exp(-I*kvec);
...
//                unitary_matrix, &ione, &ione, desca, &zero, temp, &ione, &ione, desca);
    ...
    idx = kpoints[0] * nL;
    rmg::all_reduce (ener_band, idx, COMM_EN1);
