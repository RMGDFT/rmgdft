 ...
 **    $Id: matrix_inverse_left_p.c 1242 2011-02-02 18:55:23Z luw $    **
******************************************************************************/
 ...
 */
    ...
    int ione = 1;
    int ntot, k;
    int maxrow, maxcol, *desca, *descb, *descc;
...
/*  calculate the inverse of the last block  */
    ...
    zcopy (&n1, Gii, &ione, &G_tem[n_begin1[N - 1]], &ione);
...
/*  iterate to get one more block  */
    ...
    for (i = N - 1; i > 0; i--)
    {
        ...
        descc = &pmo.desc_cond[ (i-1 + (i-1) * ct.num_blocks ) * DLEN ];
