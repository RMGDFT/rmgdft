...
void get_tf_rho (double * tf_rho)
{
	...
	xstart = tf_iptr[ion].xtal[0] / hxxgrid - dimx/2;
	xend = xstart + dimx;
	ystart = tf_iptr[ion].xtal[1] / hyygrid - dimy/2;
	yend = ystart + dimy;
	zstart = tf_iptr[ion].xtal[2] / hzzgrid - dimz/2;
	zend = zstart + dimz;
	...
	for (ix = xstart; ix < xend; ix++)
	{
		...
		for (iy = ystart; iy < yend; iy++)
		{
			...
			for (iz = zstart; iz < zend; iz++)
			{
				...
				x[0] = ix * hxxgrid - tf_iptr[ion].xtal[0];
				x[1] = iy * hyygrid - tf_iptr[ion].xtal[1];
				x[2] = iz * hzzgrid - tf_iptr[ion].xtal[2];
				r = metric (x);

				tf_rho[idx] += q * pow( (alpha/3.14159265), 1.5) * exp (-1.0*alpha*r*r);
				tf_rho[idx] -= q0 * pow( (alpha0/3.14159265), 1.5) * exp (-1.0*alpha0*r*r);
