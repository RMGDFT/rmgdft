...
GGA PBE";
    ...
    /*Pulay mixing flag, inactive unless compile flag PULAY is on*/
    get_data ("pulay_mixing", NULL, BOOL, "false");
...
Absolute";
    ...
    if (get_data ("atom_constraints", &tmp, INIT | LIST, NULL))
    {
        ...
        {
            if (sscanf
                (tbuf, "%lf %lf %lf %d", &ct.ions[tmp].constraint[0], &ct.ions[tmp].constraint[1],
                 &ct.ions[tmp].constraint[2], &ct.ions[tmp].constraint_type) != 4)
            {
            ...
            ftmp = 0.0;
            ftmp += ct.ions[tmp].constraint[0] * ct.ions[tmp].constraint[0];
            ftmp += ct.ions[tmp].constraint[1] * ct.ions[tmp].constraint[1];
            ftmp += ct.ions[tmp].constraint[2] * ct.ions[tmp].constraint[2];
            if (ftmp != ZERO)
            {
                ftmp = sqrt (ftmp);
                ct.ions[tmp].constraint[0] /= ftmp;
                ct.ions[tmp].constraint[1] /= ftmp;
                ct.ions[tmp].constraint[2] /= ftmp;
            }
        ...
        if (tmp != ct.num_ions)
        {
            printf ("Number of constrints is = %d != %d = ct.num_ions", is, ct.num_ions);
