 ...
 **    $Id: write_data.c 1127 2010-06-17 16:54:13Z luw $    **
******************************************************************************/
...
void write_data_spin (char *name, REAL * vh, REAL * rho, REAL * rho_oppo, REAL * vxc, STATE * states)
{
    ...
    nk = ct.num_kpts;
    ns = ct.num_states;
    ns_oppo = ct.num_states_oppo;   /* number of states for the opposite spin */

    write_int (fhand, &gamma, 1);
    write_int (fhand, &nk, 1);
    write_int (fhand, &ns, 1);
    write_int (fhand, &ns_oppo, 1);
    ...
    /* write the state occupations */     /* for this processor's own spin */
    {
        STATE *sp;
    ...
    /* write the state occupation for the opposite spin */

    {
        ...
        for (ik = 0; ik < nk; ik++)
            for (is = 0; is < ns_oppo; is++)
            {
                ...
                write_double (fhand, &sp->occupation_oppo, 1);
                sp++;
            }
    }

    /* write the state eigenvalues */   /* for the processor's own spin */
    {
        STATE *sp;
    ...
    /* write the eigenvalues for the opposite spin */
    {
        STATE *sp;
        ...
        for (ik = 0; ik < nk; ik++)
            for (is = 0; is < ns_oppo; is++)
            {
                ...
                write_double (fhand, &sp->eig_oppo, 1);
                sp++;
            }
    }
