...
* **Broyden** - A multi-step scheme which is the preferred option for both the Multigrid and Davidson Kohn-Sham solvers.
* **Linear** - Simple linear mixing using a constant value <b>&alpha;</b> between 0.0 and 1.0. New charge density is then calculated as <b>&rho;</b><sub>in</sub> = <b>&alpha;</b> <b>&rho;</b><sub>new</sub> + (1.0 - <b>&alpha;</b>) <b>&rho;</b><sub>old</sub>. Proper choice of <b>&alpha;</b> is crucial, for small values, reaching convergence may take many steps, while large values may lead to instability due to overshooting.
...
When applied to a single wavefunction the non-local potential operators are matrix-vector products. These are memory bandwidth bound and higher performance is obtained by applying the operators to blocks of wavefunctions. RMG reaches it's maximum performance when the block size (non_local_block_size input file option) is greater than or equal to the number of wavefunctions. This requires more memory though so the default value is 512. If you have sufficient memory setting it to a large value will provide the best performance.
