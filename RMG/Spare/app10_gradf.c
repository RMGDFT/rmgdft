 ...
 **    $Id: app10_gradf.c 1722 2012-05-07 19:38:37Z ebriggs $    **
******************************************************************************/
...
void app10_gradf (FS0_GRID * f, FP0_GRID * wx, FP0_GRID * wy, FP0_GRID * wz)
{
    ...
    my_malloc (rptr, (pct.peFPX0_GRID + 8) * (pct.peFPY0_GRID + 8) * (pct.peFPZ0_GRID + 8), REAL);
    SSSptr = (FSS0_GRID *) rptr;


    time1 = my_crtc ();
    trade_imagesx (f->s2, rptr, pct.peFPX0_GRID, pct.peFPY0_GRID, pct.peFPZ0_GRID, 4, CENTRAL_FD);
    ...
    case HEXAGONAL:
        ...
        for (ix = 2; ix < pct.peFPX0_GRID + 2; ix++)
        {

            for (iy = 2; iy < pct.peFPY0_GRID + 2; iy++)
            {

                for (iz = 2; iz < pct.peFPZ0_GRID + 2; iz++)
                {
