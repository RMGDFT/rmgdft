...
void pulay_rho(int step, int N, int N_x, int N_y, int N_z, double *rho_new, double *rho_old, int NsavedSteps, REAL ***hist, REAL ***rhist, int special_metric, REAL weight)
{
	...
	scopy(&N, rho_new, &ione, (*hist)[0], &ione);
        scopy(&N, residual, &ione, (*rhist)[0], &ione);
    }
    else
    {
        size = step + 1;
	...
	for (i = 1; i < size - 1; i++)
	    saxpy(&N, &b[i], (*hist)[i], &ione, rho_old, &ione);

	saxpy(&N, &b[size - 1], rho_new, &ione, rho_old, &ione);
	    
	/* Scale residual history by -1 * b[i] * scale and add to the result stored in rho_old*/
	for (i = 0; i < size - 1; i++)
	{
	    t1 = -1.0 * scale * b[i];
	    saxpy(&N, &t1, (*rhist)[i], &ione, rho_old, &ione);
	}

	t1 = -1.0 * scale * b[size - 1];
	saxpy(&N, &t1, residual, &ione, rho_old, &ione);
	...
	if (step > (NsavedSteps - 2))
	{
	    ...
	    scopy(&N, rho_new, &ione, (*hist)[size - 2], &ione);
	    scopy(&N, residual, &ione, (*rhist)[size - 2], &ione);
	}
	...
	{
	    scopy(&N, rho_new,  &ione, (*hist)[(size - 1)], &ione);
	    scopy(&N, residual, &ione, (*rhist)[(size - 1)], &ione);
	}
