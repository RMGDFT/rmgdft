...
double ApplyHamiltonianBlock (Kpoint<KpointType> *kptr, int first_state, int num_states, KpointType *h_psi, double *vtot, double *vxc_psi)
{
    ...
    double fd_diag = ApplyHamiltonian<KpointType, KpointType> (kptr, &kptr->Kstates[0], 0, kptr->Kstates[first_state].psi, &h_psi[first_state*pbasis_noncoll], vtot, vxc_psi, kptr->nv, false);
    ...
    for(int st1 = first_state + istop;st1 < first_state + num_states;st1++) {
        ...
        ApplyHamiltonian<KpointType, KpointType> (kptr, &kptr->Kstates[st1], st1, kptr->Kstates[st1].psi, &h_psi[st1 * pbasis_noncoll], vtot, vxc_psi, &kptr->nv[first_nls * pbasis_noncoll], false);
        first_nls++;
    }
