...
void pulay_rho(int step, int N, int N_x, int N_y, int N_z, double *rho_new, double *rho_old, int NsavedSteps, double ***hist, double ***rhist, int special_metric, double weight)
{
	...
	if (*hist == NULL)
	{
            *hist = new double *[NsavedSteps - 1];
            (*hist)[0] = new double[N * (NsavedSteps - 1)];
	...
	if (*rhist == NULL)
	{
            *rhist = new double *[NsavedSteps - 1];
            (*rhist)[0] = new double[N * (NsavedSteps - 1)];
	...
	if (special_metric) trade_space = new double[(N_x+2) * (N_y+2) * (N_z+2)];
	...
	if (special_metric) delete [] trade_space;
