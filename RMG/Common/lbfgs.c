...
*/
    ...
    if (fdstep)  // if no line minimizer is used (lineopt = 0), fdstep is alwalys 1
    {
        ...
        a1 = ddot (&tot_dim, F, &ione, Fold, &ione);
        a1 = fabs(a1);

        a2 = ddot (&tot_dim, Fold, &ione, Fold, &ione);
        a2 = fabs(a2);
...

        {
...

            {
                ...
                a1 = ddot (&tot_dim, &change_in_G[itr * tot_dim], &ione, &change_in_R[itr * tot_dim], &ione);
                ro[itr]=1.0/a1;
            }
            else
            {
                ...
                {
                    dcopy(&tot_dim, &change_in_G[(im+1) * tot_dim], &ione, &change_in_G[im * tot_dim], &ione);
                    dcopy(&tot_dim, &change_in_R[(im+1) * tot_dim], &ione, &change_in_R[im * tot_dim], &ione);
                    ro[im] = ro[im+1];
                }
                ...
                a1 = ddot (&tot_dim, &change_in_G[item * tot_dim], &ione, &change_in_R[item * tot_dim], &ione);
                ro[item] = 1.0/a1;
            }
        }  //end if(reset_flag) 

        dcopy(&tot_dim, R, &ione, Rold, &ione);
        dcopy(&tot_dim, F, &ione, Fold, &ione);

        // compute Ho*g
        bound = memory_steps;
        if (itr < memory_steps) bound = itr+1;

        dcopy(&tot_dim, F, &ione, direction, &ione);
        a1 = -1.0;
        dscal(&tot_dim, &a1, direction, &ione);

        // calculate Ho*g
        for(im = 0; im < bound; im++)
        {
            jm = bound-im -1;
            alpha_lbfgs[jm] = ro[jm]*ddot(&tot_dim, &change_in_R[jm* tot_dim], &ione, direction, &ione);
            a1 = - alpha_lbfgs[jm];
            daxpy(&tot_dim, &a1, &change_in_G[jm * tot_dim], &ione, direction, &ione);
        }


        a1 = invcurv;
        dscal(&tot_dim, &a1, direction, &ione); //Ho=Ho*invcurv Ho is identity matrix


        for(im = 0; im < bound; im++)
        {
            beta = ro[im]*ddot(&tot_dim, &change_in_G[im* tot_dim], &ione, direction, &ione);
            a1 =  alpha_lbfgs[im] - beta;
            daxpy(&tot_dim, &a1, &change_in_R[im * tot_dim], &ione, direction, &ione);
        }
        ...
        if (lineopt) // line minimizer (using fdstep)
        {
            ...
            a1 = ddot (&tot_dim, direction, &ione, direction, &ione);
            a1 = 1.0/sqrt(a1);
...

    {
        ...
        fp1 = ddot (&tot_dim, Fold, &ione, direction, &ione);
        fp2 = ddot (&tot_dim, F, &ione, direction, &ione);
