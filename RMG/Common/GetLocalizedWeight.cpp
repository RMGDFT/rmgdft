...
template <class KpointType> void Kpoint<KpointType>::GetLocalizedWeight (void)
{
    ...
    beptr = new std::complex<double>[2*max_size];
...
// Used for localizing projectors in real space
template void Kpoint<double>::GetLocalizedWeight_xyz(void);
template void Kpoint<std::complex<double>>::GetLocalizedWeight_xyz(void);

template <class KpointType> void Kpoint<KpointType>::GetLocalizedWeight_xyz (void)
{
    ...
    for(int ixyz = 0; ixyz < 3; ixyz++)
    {
        for (int ion1 = 0; ion1 < num_nonloc_ions; ion1++)
        {
            ...
            //Nlweight: pointer to x|beta> , y|beta>, z|beta>
            Nlweight = &nl_weight[ion1 * ct.max_nl * P0_BASIS + (ixyz+1) * this->nl_weight_size];
            ...
            /*Temporary pointer to the already calculated forward transform */
            fptr = (std::complex<double> *)&sp->forward_beta_r[ixyz][kidx*sp->num_projectors * coarse_size];
