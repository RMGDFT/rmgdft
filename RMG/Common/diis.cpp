...
template <class T> void diis<T>::compute_lambda(double eig, T *iHu, T *ir0, T *Hr0)
{
    ...
    if constexpr (std::is_same_v<T, double>)
    {
        ...
        rmg::all_reduce(ss, 2, pct.coalesced_grid_comm);
        lambda = ss[0]/ss[1];
     }
...
template <class T> std::vector<T> diis<T>::compute_estimate()
{
    ...
    rmg::all_reduce((double *)A.data(), factor*M*M, pct.coalesced_grid_comm);
...
template <class T> T diis<T>::dot(std::vector<T>& a, std::vector<T>& b)
{
    ...
    rmg::all_reduce((double *)&sum, factor, pct.coalesced_grid_comm);
    return sum;
}
