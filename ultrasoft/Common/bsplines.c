...
void bspline_interp_full (REAL * rho, REAL * rho_f)
{
    ...
    REAL *bscoef, *crdsx, *crdsy, *crdsz, t1;
    REAL cspacing_x, cspacing_y, cspacing_z, fspacing_x, fspacing_y, fspacing_z;
    REAL offset_x, offset_y, offset_z;

    /*These variables should keep values between calls */
#if MD_TIMERS
    static REAL time, time1;
#endif
    static int flag = 0;
    static REAL *biatx, *biaty, *biatz, *knots_x, *knots_y, *knots_z;
    ...
    cmax_x = (PX0_GRID + 2 * ct.interp_trade);
    cmax_y = (PY0_GRID + 2 * ct.interp_trade);
    cmax_z = (PZ0_GRID + 2 * ct.interp_trade);

    my_malloc (bscoef, cmax_x * cmax_y * cmax_z, REAL);

    /*This will hold coordinates of the grid points */
    my_malloc (crdsx, FPX0_GRID, REAL);
    my_malloc (crdsy, FPY0_GRID, REAL);
    my_malloc (crdsz, FPZ0_GRID, REAL);

    if (!flag)
    {

        /*Memory for knots and B-spline coefficients */
        my_malloc (knots_x, cmax_x + xorder, REAL);
        my_malloc (knots_y, cmax_y + yorder, REAL);
        my_malloc (knots_z, cmax_z + zorder, REAL);

        my_malloc (biatx, FPX0_GRID * xorder, REAL);
        my_malloc (biaty, FPY0_GRID * yorder, REAL);
        my_malloc (biatz, FPZ0_GRID * zorder, REAL);

        my_malloc (leftx, FPX0_GRID, int);
        my_malloc (lefty, FPY0_GRID, int);
        my_malloc (leftz, FPZ0_GRID, int);
    }



    /*Setup crds for knots */
    t1 = 1.0 / (REAL) (cmax_x - 1);
    for (ix = 0; ix < cmax_x; ix++)
        crdsx[ix] = (REAL) ix *t1;

    t1 = 1.0 / (REAL) (cmax_y - 1);
    for (ix = 0; ix < cmax_y; ix++)
        crdsy[ix] = (REAL) ix *t1;

    t1 = 1.0 / (REAL) (cmax_z - 1);
    ...
    my_malloc (rho_c, cmax_x * cmax_y * cmax_z, REAL);
    trade_imagesx (rho, rho_c, PX0_GRID, PY0_GRID, PZ0_GRID, ct.interp_trade);
    ...
    if (!flag)
    {
        ...
        cspacing_x = 1.0 / (REAL) (cmax_x - 1);
        cspacing_y = 1.0 / (REAL) (cmax_y - 1);
        cspacing_z = 1.0 / (REAL) (cmax_z - 1);

        /*Fine grid spacing */
        fspacing_x = cspacing_x / FG_NX;
        fspacing_y = cspacing_y / FG_NX;
        fspacing_z = cspacing_z / FG_NX;
        ...
        get_biats (FPX0_GRID, crdsx, FPY0_GRID, crdsy, FPZ0_GRID, crdsz,
...
#if MD_TIMERS
    time1 = my_crtc ();
    rmg_timings (INTERP_SETUP_TIME, time1 - time, 0);
#endif

    dbs3gd2 (FPX0_GRID, FPY0_GRID, FPZ0_GRID,
             xorder, yorder, zorder,
             cmax_x, cmax_y, cmax_z, bscoef,
             rho_f, FPX0_GRID, FPY0_GRID, FPZ0_GRID, biatx, biaty, biatz, leftx, lefty, leftz);


#if MD_TIMERS
    rmg_timings (INTERP_EVAL_TIME, my_crtc () - time1, 0);
#endif
