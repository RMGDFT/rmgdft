...
static size_t linked_list_mem = 0;
...
static void initialize (void *ptr, size_t n, char *vn, char *type, char *fn, int ln)
{
    ...
    {
        printf
            ("!!!! warning: requested initialization at '%s:%d' of data pointed to by '%s' not done.\n",
             fn, ln, vn);
        printf
            ("              please inplement initialization procedure for type '%s' in '%s:%d'.\n",
...
static void *alloc (size_t nobj, size_t size1,
                    char *vn, char *type, char *fn, int ln, int flag, int init)
{
    void *ptr = NULL;
    ...
    if (nobj > 0)
    {
        ...
        if (flag == ALLOC_M)
            ptr = malloc (nobj * size1);
        else if (flag == ALLOC_C)
            ptr = calloc (nobj, size1);
        else
            ptr = NULL;
...
void *salloc (size_t nobj, size_t size1, char *vn, char *type, char *fn, int ln, int flag, int init)
{

#if ! USE_SALLOC

    return alloc (nobj, size1, vn, type, fn, ln, flag, init);
    ...
    node = (salloc_nodep_t) alloc (1, sizeof (salloc_node_t),
    ...
    /* allocate memory */
    node->ptr = alloc (nobj, size1, vn, type, fn, ln, flag, init);

    /*Increment memory counter for the allocated memory as well as the node
     * memory required to keep tracking info*/
    totalmem += node->size + sizeof (salloc_node_t);    /* size in bytes */
    /*This tracks only memory of the linked list */
    linked_list_mem += sizeof (salloc_node_t);
    ...
    if (ptr != NULL)
    {
        ...
        do
        {
            ...
            if (ptr == node->ptr)
            {
                ...
                /*Decremement memory counter for the released memory as well as the node
                 * memory required to keep tracking info*/
                totalmem -= (node->size + sizeof (salloc_node_t));
                /*This tracks only memory of the linked list */
                linked_list_mem -= sizeof (salloc_node_t);
...
void salloc_report (char *fn, int ln, int ipe, char *tag)
{
    ...
    for (jpe = 0; jpe < npe; jpe++)
    {
        if (jpe == ipe)
        {
            ...
            fprintf (repf, "\n");
            fprintf (repf,
                     "pe=%3d total of %10d b not deallocated including %10d b required to track this memory\n",
                     ipe, (int) totalmem, (int) linked_list_mem);
