 ...
 **    $Id: init_kbr.c 779 2007-05-14 19:30:52Z luw $    **
******************************************************************************/
 ...
 *   nothing
 * OUTPUT
 *   projectors are stored in structure SPECIES (see md.h)
 * PARENTS
 *   init.c
 * CHILDREN
 *   rft1.c radiff.c 
...
void init_kbr(void)
{
    ...
    for (isp = 0; isp < ct.num_species; isp++)
    {
        ...
        rft1(ct.cparm, work, &sp->r[0], sp->localig, &sp->rab[0],
             sp->rg_points, 0, sp->drlig, sp->gwidth, MAX_LOCAL_LIG);
        ...
        for (ip = 0; ip < sp->nbeta; ip++)
        {
            ...
            /* Transform to g-space and filter it */
            rft1(ct.betacparm, &sp->beta[ip][0], &sp->r[0],
                 &sp->betalig[ip][0], &sp->rab[0], sp->rg_points,
                 sp->llbeta[ip], sp->drnlig, sp->gwidth, MAX_LOCAL_LIG);
