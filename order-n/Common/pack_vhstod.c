 ...
 **    $Id: pack_vhstod.c 587 2006-08-18 22:57:56Z miro $    **
******************************************************************************/
...
void pack_vhstod(REAL * s, REAL * d, int dimx, int dimy, int dimz)
{
    ...
    pe2xyz(pct.coords[1], &pex, &pey, &pez);
    sxlo = pex * dimx;
    sylo = pey * dimy;
    szlo = pez * dimz;
    sxhi = sxlo + dimx - 1;
    ...
    dxlo = pex * 2 * dimx - ct.vh_nxgrid / 2;
    dylo = pey * 2 * dimy - ct.vh_nygrid / 2;
    dzlo = pez * 2 * dimz - ct.vh_nzgrid / 2;
    dxhi = dxlo + 2 * dimx - 1;
    dyhi = dylo + 2 * dimy - 1;
    dzhi = dzlo + 2 * dimz - 1;
    hincy = 2 * dimz;
