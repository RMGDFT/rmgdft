...
#include <string>
 ...
 std::unordered_map<std::string, int> bravais_lattice_type = {
        {"None", 0},
        {"Cubic Primitive", 1},
...
//        {"Monoclinic Base Centered", 13},
 ...
 std::unordered_map<std::string, int> drho_precond_type = {
        {"Resta", PRECOND_RESTA},
        {"Kerker", PRECOND_KERKER}};

 std::unordered_map<std::string, int> crds_units = {
        {"Bohr", 0},
        {"Angstrom", 1}};

 std::unordered_map<std::string, int> vdw_corr = {
        {"None", 0},
        {"DFT-D2", 1},
        {"Grimme-D2", 1},
        {"DFT-D3", 2},
 ...
 std::unordered_map<std::string, int> lattice_units = {
        {"Bohr", 0},
        {"Angstrom", 1},
        {"Alat", 2}};

 std::unordered_map<std::string, int> charge_mixing_type = {
        {"Linear", 0},
        {"Pulay", 1},
        {"Broyden", 2},
        {"Auto", 3}};

 std::unordered_map<std::string, int> charge_analysis = {
        {"None", 0},
        {"Voronoi", 1}};

 std::unordered_map<std::string, int> vdwdf_grid_type = {
        {"Coarse", 0},
        {"Fine", 1}};

 std::unordered_map<std::string, int> relax_mass = {
        {"Atomic", 0},
        {"Equal", 1}};

 std::unordered_map<std::string, int> md_temperature_control = {
        {"Nose Hoover Chains", 0},
        {"Anderson Rescaling", 1}};

 std::unordered_map<std::string, int> atomic_coordinate_type = {
        {"Cell Relative", 0},
        {"Absolute", 1}};

 std::unordered_map<std::string, int> calculation_mode = {
        {"Quench Electrons", 0},
        {"Relax Structure", 1},
 ...
 std::unordered_map<std::string, int> dos_method = {
        {"tetrahedra", 0},
        {"Gaussian", 1},
        };

 std::unordered_map<std::string, int> tetra_method = {
        {"Bloechl", 0},
        {"Linear", 1},
        {"Optimized", 2}
        };

 std::unordered_map<std::string, int> occupations_type = {
        {"Fixed", 0},
        {"Fermi Dirac", OCC_FD},
        {"Gaussian", OCC_GS},
 ...
 std::unordered_map<std::string, int> subdiag_driver = {
 ...
 std::unordered_map<std::string, int> kohn_sham_solver = {
        {"multigrid", MULTIGRID_SOLVER},
        {"davidson", DAVIDSON_SOLVER}};

 std::unordered_map<std::string, int> force_derivate_type = {
        {"wavefunction", WAVEFUNCTION_DERIVATIVE},
        {"projector", PROJECTOR_DERIVATIVE}};

 std::unordered_map<std::string, int> poisson_solver = {
        {"multigrid", MULTIGRID_SOLVER},
        {"pfft", POISSON_PFFT_SOLVER}};

 std::unordered_map<std::string, int> kpoint_units = {
        {"Reciprocal lattice", KPOINT_LATT_UNIT},
        {"2pi/alat", KPOINT_2pi_alat}};
 
 std::unordered_map<std::string, int> md_integration_order = {
        {"2nd Velocity Verlet", 0},
        {"3rd Beeman-Velocity Verlet", 1},
        {"5th Beeman-Velocity Verlet", 2}};

 std::unordered_map<std::string, int> interpolation_type = {
        {"prolong", PROLONG_INTERPOLATION},
        {"FFT", FFT_INTERPOLATION}};

 std::unordered_map<std::string, int> start_mode = {
 ...
 std::unordered_map<std::string, int> ldaU_mode = {
        {"None", LDA_PLUS_U_NONE},
        {"Simple", LDA_PLUS_U_SIMPLE}};

 std::unordered_map<std::string, int> boundary_condition_type = {
        {"Periodic", 0}};
//        {"Cluster", 1},
//        {"Surface", 2}};

 std::unordered_map<std::string, int> z_average_output_mode = {
        {"None", 0},
...
// immediately following
 std::unordered_map<std::string, int> exchange_correlation_type = {
        {"pz", 0},
        {"PZ", 0},
        {"LDA", 0},
        {"bp", 1},
...
// Internal types accepted by QE routines. Must map to preceding array
 ...
 std::unordered_map<std::string, int> relax_method = {
        {"Fast Relax", 0},
        {"FIRE", 1},
        {"Quick Min", 2},
        {"MD Min", 3},
        {"LBFGS", 4}};
 std::unordered_map<std::string, int> mg_method = {
        {"Steepest Descent", 0},
        {"Pulay", 1},
        {"Kain", 2}
        };
 std::unordered_map<std::string, int> orbital_layout = {
        {"Distribute", 0},
        {"Projection", 1}
        };

 std::unordered_map<std::string, int> energy_point_insert_mode = {
        {"None", 0},
        {"Simpson", 1},
        {"Sharp Peaks", 2}
        };

 std::unordered_map<std::string, int> exx_mode = {
        {"Distributed fft", EXX_DIST_FFT},
        {"Local fft", EXX_LOCAL_FFT},
        };

 std::unordered_map<std::string, int> exxdiv_treatment = {
