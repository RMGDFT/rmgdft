...
void ReadCommon(int argc, char *argv[], char *cfile)
{
    ...
    control.add_options()
       //("description", po::value<std::string>(&ii)->default_value("RMG RUN"), "description of run")
        ...
        ("charge_pulay_scale", po::value<double>(&ct.charge_pulay_scale)->default_value(0.50), "")
        ("charge_pulay_special_metrics_weight", po::value<double>(&ct.charge_pulay_special_metrics_weight)->default_value(100.0), "")
        ("charge_density_mixing", po::value<double>(&ct.mix)->default_value(0.5), "")
        ("rms_convergence_criterion", po::value<double>(&ct.thr_rms)->default_value(1.0E-7), "")
        ("relax_max_force", po::value<double>(&ct.thr_frc)->default_value(2.5E-3), "")
        ("occupation_electron_temperature_eV", po::value<double>(&ct.occ_width)->default_value(0.04), "")
        ("occupation_number_mixing", po::value<double>(&ct.occ_mix)->default_value(0.3), "")
        ("electric_field_magnitude", po::value<double>(&ct.e_field)->default_value(0.0), "")
        ("ionic_time_step", po::value<double>(&ct.iondt)->default_value(50), "")
        ("hartree_rms_ratio", po::value<double>(&ct.hartree_rms_ratio)->default_value(1000.0), "")
        ...
        ("kohn_sham_time_step", po::value<double>(&ct.eig_parm.gl_step)->default_value(0.3), "")
        ("poisson_finest_time_step", po::value<double>(&ct.poi_parm.gl_step)->default_value(0.6), "")
        ("poisson_coarse_time_step", po::value<double>(&ct.poi_parm.sb_step)->default_value(0.6), "")
        ("energy_cutoff_parameter", po::value<double>(&ct.cparm)->default_value(1.75), "")
        ("potential_acceleration_constant_step", po::value<double>(&ct.potential_acceleration_constant_step)->default_value(0.0), "")
        ("potential_acceleration_poisson_step", po::value<double>(&ct.potential_acceleration_poisson_step)->default_value(0.0), "")
        ("projector_mixing", po::value<double>(&ct.prjmix)->default_value(0.5), "")
        ("Emin", po::value<double>(&ct.Emin)->default_value(-6.0), "")
        ("Emax", po::value<double>(&ct.Emax)->default_value(0.0), "")
        ("folded_spectrum_width", po::value<double>(&ct.folded_spectrum_width)->default_value(0.3), "")
        ("neb_spring_constant", po::value<double>(&ct.neb_spring_constant)->default_value(0.5), "")
        ("md_temperature", po::value<double>(&ct.nose.temp)->default_value(300), "")
        ("md_nose_oscillation_frequency_THz", po::value<double>(&ct.nose.fNose)->default_value(15.59), "")
        ("ionic_time_step", po::value<double>(&ct.iondt)->default_value(50), "")
        ("max_ionic_time_step", po::value<double>(&ct.iondt_max)->default_value(150), "")
        ("ionic_time_step_increase", po::value<double>(&ct.iondt_inc)->default_value(1.1), "")
//        ("ionic_time_step_decrease", po::value<double>(&ct.iondt_dec) DBL, s_ttt);
        ("system_charge", po::value<double>(&ct.background_charge)->default_value(0), "")
        ("energy_cutoff_parameter", po::value<double>(&ct.cparm)->default_value(1.75), "")

#if GPU_ENABLED
        ("gpu_direct_collectives", po::value<bool>(&ct.gpu_direct_collectives)->default_value(false), "")
#endif
...
//        ("write_pseudopotential_plots", po::value<bool>(NULL)->default_value(false), "")
        ...
        ("mask_function_filtering", po::value<bool>(&ct.mask_function)->default_value(false), "")
        ("write_memory_report", po::value<bool>(&ct.write_memory_report)->default_value(false), "")
...
//        ("relax_dynamic_timestep", po::value<bool>(NULL)->default_value(false), "")
        ("write_pdos", po::value<bool>(&ct.pdos_flag)->default_value(false), "")
// Depends on start flag
//        ("md_randomize_velocity", po::value<bool>(&ct.nose.randomvel)->default_value(false), "")
//        ("md_randomize_velocity", po::value<bool>(&ct.nose.randomvel)->default_value(true), "")
        ("scalapack_global_sums", po::value<bool>(&ct.scalapack_global_sums)->default_value(true), "")
    ;
