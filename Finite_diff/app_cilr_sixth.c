...
rmg_double_t app_cilr_sixth_global (rmg_double_t * psi, rmg_double_t *a_psi, rmg_double_t *b_psi, rmg_double_t *vtot_eig_s,
...
rmg_double_t app_cilr_sixth_global (rmg_double_t * rptr, rmg_double_t *a_psi, rmg_double_t *b_psi, rmg_double_t *vtot_eig_s,
                           rmg_double_t gridhx, rmg_double_t gridhy, rmg_double_t gridhz)
{
    ...
    for (ix = 2; ix < FIXED_XDIM + 2; ix++)
    {
        ...
        for (iy = 2; iy < FIXED_YDIM + 2; iy++)
        {
            ...
            for (iz = 2; iz < FIXED_ZDIM + 2; iz+=4)
            {
                ...
                // Second
                a_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz - 1)] = cc * rptr[ixs + iys + iz + 1] +
                    fcx * rzps +
                    fcx * (rptr[ixs + iys + iz] + rptr[ixs + iys + iz + 2]);

                a_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz - 1)] +=
                    ecxz * rz +  ecxz * rzpps +  ecxz * rfc1;

                // Second
                b_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz - 1)] =
                ...
                // Third
                a_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz)] = cc * rptr[ixs + iys + iz + 2] +
                    fcx * rzpps +
                    fcx * (rptr[ixs + iys + iz + 1] + rptr[ixs + iys + iz + 3]);

                a_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz)] +=
                    ecxz * rzps + ecxz * rzms + ecxz * rfc1;

                b_psi[(ix - 2) * incxr + (iy - 2) * incyr + iz] =
                    c100 * (rptr[ixs + iys + iz+1] +
                            rptr[ixs + iys + iz + 3] +
                            rzpps) +
                ...
                // Fourth
                a_psi[(ix - 2) * incxr + (iy - 2) * incyr + (iz + 1)] = cc * rptr[ixs + iys + iz + 3] +
                    fcx * rzms +
