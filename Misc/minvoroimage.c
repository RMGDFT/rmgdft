...
double minvoroimage (ION * ip1, double gridcoor[3])
{
    ...
    double r[27], ax[3], rmin, x[27], y[27], z[27], xtal_r[3];
    ...
    if (ct.boundaryflag == CLUSTER)
    {
        ...
        xtal_r[0] = ip1->xtal[0] - gridcoor[0];
        xtal_r[1] = ip1->xtal[1] - gridcoor[1];
        xtal_r[2] = ip1->xtal[2] - gridcoor[2];
    ...
    for (ix = -1; ix <= 1; ix++)
    {
        ...
        for (iy = -1; iy <= 1; iy++)
        {
            ...
            for (iz = -1; iz <= 1; iz++)
            {
                ...
                x[idx] = ip1->xtal[0] - gridcoor[0] + (double) ix;
                y[idx] = ip1->xtal[1] - gridcoor[1] + (double) iy;
                z[idx] = ip1->xtal[2] - gridcoor[2] + (double) iz;
                if (ct.boundaryflag == SURFACE)
                {

                    z[idx] = ip1->xtal[2] - gridcoor[2];
