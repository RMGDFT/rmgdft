...
MODULE symm_base
  ...
  !
  !INTEGER :: s(3,3,48)
  !INTEGER, bind(C) :: s(3,3,48)
  INTEGER(C_INT), ALLOCATABLE, TARGET :: s(:,:,:)
...
!write(6,*)'AT ',at(1,1),'  ',at(2,1),'  ',at(3,1)
!write(6,*)'AT ',at(1,2),'  ',at(2,2),'  ',at(3,2)
!write(6,*)'AT ',at(1,3),'  ',at(2,3),'  ',at(3,3)
...
!write(6,*)'BG ',bg(1,3),'  ',bg(2,3),'  ',bg(3,3)
   ...
   10   CONTINUE
     ...
     IF ( nrot /= 1 .AND. nrot /= 2 .AND. nrot /= 4 .AND. nrot /= 6 .AND. &
          ...
          WRITE (6, '(80("-"),/,"NOTICE: Bravais lattice has wrong number (",&
     ...
     IF ( .NOT. is_group(nrot) ) THEN
        ...
        write(6,*) 'set_sym_bl NOTICE: Symmetry group for Bravais lattice &
                     is not a group (',nrot, ') - symmetries are disabled' 
         nrot = 1
     ENDIF
     !
     nrot_out = nrot
   ...
   SUBROUTINE find_sym( nat, tau, ityp, magnetic_sym_in, m_loc, no_z_inv_in, nsym_out )
     ...
     INTEGER, INTENT(IN) :: magnetic_sym_in
     ...
     IF(magnetic_sym_in .NE. 0) THEN
         magnetic_sym = .TRUE.
     ENDIF

     IF(no_z_inv_in .NE. 0) THEN
         no_z_inv = .TRUE.
     ENDIF
...
!write(6,*)'NROT1 in copy_sym = ',nrot,'   ',nsym
       ...
       IF ( .NOT. is_group( nsym ) ) THEN
          ...
          accep = accep * 0.5d0
          IF (i == 3) THEN
            write(6,*)'find_sym Still not a group! symmetry disabled'
            nsym = 1
          ENDIF
          CYCLE symm
       ELSE
          IF (i > 1) write(6,*)'find_sym Symmetry operations form a group'
          EXIT symm
       ENDIF
     ENDDO symm
     !
...
write(6,*)xau(:,na)
   ...
   !
   !-------------------------------------------------------------------------
   SUBROUTINE set_sym( nat, tau, ityp, nspin_mag, m_loc, at, bg )
     !-----------------------------------------------------------------------
     ...
     INTEGER dumrot, time_reversal_in, no_z_inv_in
    ...
    FUNCTION check_grid_sym ( nr1, nr2, nr3 ) RESULT ( compatible )
      ...
      DO isym = 1, nsym
         ...
         IF ( bad ) THEN
            ...
            WRITE( 6, '(3i4)') ( (s(i,j,isym), j=1,3), i=1,3 )
            compatible = .false.
         ENDIF
         !
      ENDDO
