...
double  MadelungConstant()
{
    ...
    //  check how many unit cell to make erfc(nL/*sigma) < tol ==1.0e-8
    //  erfc(4.06) = 9.37e-9 so we set the largest r/sigma) = 4.06


    double rcutoff = 4.06;
    int num_cell_x = int(rcutoff /(kappa * Rmg_L.get_xside())) + 1;
    int num_cell_y = int(rcutoff /(kappa * Rmg_L.get_yside())) + 1;
    int num_cell_z = int(rcutoff /(kappa * Rmg_L.get_zside())) + 1;


    double vol = Rmg_L.get_omega();
    double madelung_c = -PI/(kappa * kappa * vol) - 2.0 * kappa/sqrt(PI);
    ...
    for(int ix = -num_cell_x; ix<= num_cell_x; ix++)
    {
        for(int iy = -num_cell_y; iy<= num_cell_y; iy++)
        {
            for(int iz = -num_cell_z; iz<= num_cell_z; iz++)
            {
                x =  ix * Rmg_L.a0[0] + iy * Rmg_L.a1[0] + iz * Rmg_L.a2[0];
                y =  ix * Rmg_L.a0[1] + iy * Rmg_L.a1[1] + iz * Rmg_L.a2[1];
                z =  ix * Rmg_L.a0[2] + iy * Rmg_L.a1[2] + iz * Rmg_L.a2[2];
                r = sqrt(x*x + y*y + z*z);

                // r /= 0 
                if(r > 1.0e-5) madelung_c +=  boost::math::erfc(r*kappa)/r;
    ...
    for(size_t ig=0;ig < ewald_pwaves->pbasis;ig++)
    {
        if(ewald_pwaves->gmags[ig] > 1.0e-6)
        {
            ...
            madelung_c_g += 4.0 * PI * exp(-gsquare/(4.0*kappa * kappa))/ gsquare/vol;
        }
    }

    MPI_Allreduce(MPI_IN_PLACE, &madelung_c_g, 1, MPI_DOUBLE, MPI_SUM, pct.grid_comm);
    madelung_c += madelung_c_g;
    return madelung_c;
}
