...
/*              work3((2*kz-1)*nz) */
    ...
    work1 = new double[nx * ny * nz];
    work2 = new double[nz];
    work3 = new double[(2 * kz - 1) * nz];
                ...
                double *biatx, double *biaty, double *biatz, int *leftx, int *lefty, int *leftz)
{
    ...
    dl = new double[maxnvec * maxk];
    dr = new double[maxnvec * maxk];
    term = new double[maxnvec];
    save1 = new double[maxnvec];
...
ct.interp_trade = 7;
    ...
    bscoef = new double[cmax_x * cmax_y * cmax_z];

    /*This will hold coordinates of the grid points */
    crdsx = new double[get_FPX0_GRID()];
    crdsy = new double[get_FPY0_GRID()];
    crdsz = new double[get_FPZ0_GRID()];

    if (!flag)
    {

        /*Memory for knots and B-spline coefficients */
        knots_x = new double[ cmax_x + xorder];
        knots_y = new double[ cmax_y + yorder];
        knots_z = new double[ cmax_z + zorder];

        biatx = new double[ get_FPX0_GRID() * xorder];
        biaty = new double[ get_FPY0_GRID() * yorder];
        biatz = new double[ get_FPZ0_GRID() * zorder];

        leftx = new int[ get_FPX0_GRID()];
        lefty = new int[ get_FPY0_GRID()];
        leftz = new int[ get_FPZ0_GRID()];
    }
    ...
    rho_c = new double[cmax_x * cmax_y * cmax_z];
    Rmg_T->trade_imagesx (rho, rho_c, get_PX0_GRID(), get_PY0_GRID(), get_PZ0_GRID(), ct.interp_trade, FULL_TRADE);
