...
void Free2DArray(double **array, const size_t rows, const size_t cols) {
...
void SimplePolyFit(const double *x, double *y, const size_t n, const size_t k, double *beta)
{
    ...
    CalculateWeights(erry, Weights, n, 0);

    PolyFit(x, y, n, k, false, 0.0, beta,  Weights, XTWXInv);

    delete [] erry;
    Free2DArray(Weights, n, n);
    Free2DArray(XTWXInv, k+1, k+1);
}
